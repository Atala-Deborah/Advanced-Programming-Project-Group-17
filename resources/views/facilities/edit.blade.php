...
<div class="max-w-2xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        ...
        <form action="{{ route('facilities.update', $facility) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="Description" id="Description" rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('Description', $facility->Description) }}</textarea>
                    @error('Description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="FacilityType" id="FacilityType" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        <option value="Lab" {{ old('FacilityType', $facility->FacilityType) == 'Lab' ? 'selected' : '' }}>Lab</option>
                        <option value="Workshop" {{ old('FacilityType', $facility->FacilityType) == 'Workshop' ? 'selected' : '' }}>Workshop</option>
                        <option value="Testing Center" {{ old('FacilityType', $facility->FacilityType) == 'Testing Center' ? 'selected' : '' }}>Testing Center</option>
                    </select>
                    @error('FacilityType')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="Capabilities" id="Capabilities" rows="2"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('Capabilities', $facility->Capabilities) }}</textarea>
                    @error('Capabilities')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
